...
for (i in 1:1000){
  ...
  rand_S_HONS <-specnumber(HONS)
  rand_S_HOS <-specnumber(HOS)
  rand_S_HNS <-specnumber(HNS)
  rand_S_HS <-specnumber(HS)
...
write.csv(p.values,file="Pvalues_totaldiversity_Gil.csv")
write.csv(mean_data,file="totaldiversity_data_gil.csv")
